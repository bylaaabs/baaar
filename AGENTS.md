...
Only the maintainer cuts releases. The full procedure is in [docs/RELEASING.md](docs/RELEASING.md);
the short version:

- `scripts/release/release.sh <version> [--dry-run]` does everything from this Mac: a Developer ID
  signed, hardened, notarized and stapled `dist/baaar.app`, then `dist/baaar-v<version>.dmg` and
  `.zip`, checksums and the Homebrew cask, the signed tag `v<version>` and the GitHub release with
  the CHANGELOG section as notes. The pieces (`build-release.sh`, `notarize.sh`, `make-dmg.sh`,
  `checksum.sh`) run on their own too.
- `.github/workflows/release.yml` runs the same scripts when a `v*` tag is pushed, once the signing
  secrets are stored. It never replaces a release that already exists.
- Before a release: `CHANGELOG.md` has a `## [v<version>] - <date>` section (it becomes the release
  notes, so it obeys the voice rules), `project.yml` has the same `MARKETING_VERSION`, and `main` is
  green and has been used. `release.sh` refuses otherwise, and with a dirty tree or an existing tag.
- Tags are immutable. A broken release gets the next pre-release number, never a moved tag.
- Agents never run `notarize.sh`, `release.sh` without `--dry-run`, `gh release`, or `git tag` on
  their own. Building and packaging (`build-release.sh`, `make-dmg.sh`, `--dry-run`) are fine.
- Signing material stays out of the repository and out of the conversation: no `.p12`, no `.p8`,
  no keychain passwords, no App Store Connect ids in files or commits.
- `Resources/dmg-background.png` is generated by `scripts/render-dmg-background.swift`; edit the
  script, re-run it, commit both. Its icon coordinates match `scripts/release/make-dmg.sh`.
